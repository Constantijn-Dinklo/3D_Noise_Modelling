...
class ReflectionPath:
    ...
    def split_lineseg_n(self, n, lineseg):
        ...
        """
        delta_x = lineseg[1][0] - lineseg[0][
            0]  # delta_x can be positive, negative or zero, depending on the direction of the line.
        delta_y = lineseg[1][1] - lineseg[0][
            1]  # delta_x can be positive, negative or zero, depending on the direction of the line.
        vertex = lineseg[0]
        ref_list = [vertex]
    ...
    def get_second_paths(self, s, r, t):
        ...
        for candidate in c_list:
            for bag_id in f_dict:
                ...
                walls = f_dict[bag_id]['walls']
                for wall in walls:
                    test_c = misc.side_test(wall[0], wall[1], candidate[1][
                                                              :2])  # r[:2] makes the function to ignore an eventual 'z' value.
                    test_s = misc.side_test(wall[0], wall[1],
                                            s[:2])  # s[:2] makes the function to ignore an eventual 'z' value.
                    if test_c > 0 and test_s > 0:  # This statement guarantees that S-REF and REF-CANDIDATE are entirely outside the polygon.
                        ...
                        if type(b) == list:
                            ...
                            if test_b > 0 and test_r > 0:
                                ...
                                dist = math.sqrt(
                                    ((b_mirror[0] - candidate[1][0]) ** 2) + ((b_mirror[1] - candidate[1][1]) ** 2))
                                if dist > 0.1:
                                    if abs(misc.side_test(b_mirror, candidate[1][:2], r)) <= t:
                                        # GET CLOSEST POINT FROM R TO B_MIRROR_CANDIDATE[1]
                                        r_closest = self.get_closest_point(r, (
                                            self.get_line_equation(b_mirror, candidate[1])))
        ...
        return [coords,
                heights]  # [ [ [p11, p12], [p21, p22], .... [pn1, pn2] ] , [ [h11, h12], [h21, h22], .... [hn1, hn2] ]
...
def write_output_2nd(output_file, lista):
    ...
    for path in lista:
        ...
        count, sou[0], sou[1], sou[2], b_ref[0], b_ref[1], b_ref[2], c_ref[0], c_ref[1], c_ref[2], rec[0], rec[1],
        rec[2])
        fout.write(line)
    fout.close()
    # MultiLineStringZ ((93528.02305619 441927.11005859 2.5, 93567.67848824 441908.81858497 0, 93539.68248698 441892 1.4))
...
if __name__ == "__main__":
    ...
    read_buildings(
        '//Users/denisgiannelli/Documents/DOCS_TU_DELFT/_4Q/GEO1101/06_DATA/03_midpresentation/buildings_lod_13.gpkg',
        f_dict)
    read_points('//Users/denisgiannelli/Documents/DOCS_TU_DELFT/_4Q/GEO1101/06_DATA/03_midpresentation/sources.gpkg',
                s_dict)
    read_points('//Users/denisgiannelli/Documents/DOCS_TU_DELFT/_4Q/GEO1101/06_DATA/03_midpresentation/receivers.gpkg',
                r_dict)
    ...
    write_candidates(
        '//Users/denisgiannelli/Documents/DOCS_TU_DELFT/_4Q/GEO1101/06_DATA/03_midpresentation/candidates0025.csv',
        c_list)
    ...
    write_output_1st(
        '//Users/denisgiannelli/Documents/DOCS_TU_DELFT/_4Q/GEO1101/06_DATA/03_midpresentation/path_1st.csv', p1_list)
    write_output_2nd(
        '//Users/denisgiannelli/Documents/DOCS_TU_DELFT/_4Q/GEO1101/06_DATA/03_midpresentation/path_2nd_dim00025_t01.csv',
        p2_list)
