...
class Tin:
    ...
    def create_cross_section_dtm(self, edge_list, source, receiver, source_tr, receiver_tr):
        """
        Explanation:
        ---------------
        Input:
    ...
    def create_cross_section_dsm(self, dtm_vertices, dtm_edges, buildings):
        ...
        for edge in dtm_edges:
            ...
            if edge[1] == -1:
                # wrong append vertices will be inserted twice
...
def main(sys_args):
    ...
    building = fiona.open(r"C:\Users\Nadine\Desktop\Nad\TU delft\Q4\GEO1101\3d_geo_data\lod13\lod13.shp")
    first = building.next()
    random = building.get(10)
    shp_geom = shape(random['geometry'])
    h = random['properties']['h_dak']
    flat_shp_geom = transform(lambda x, y, z=None: (x, y), shp_geom)
    brep = shp_geom.boundary
    base_h = list(brep.coords)[0][2]
    twod_brep = transform(lambda x, y, z=None: (x, y), brep)
    polygon = [(0.0, 0.0), (3.0, 0.0), (3.0, 2.0), (2.0, 2.0), (2.0, 1.0), (1.0, 1.0), (1.0, 2.0), (0.0, 2.0),
               (0.0, 0.0)]
    shapely_poly = Polygon(polygon)
    brep_test = shapely_poly.boundary

    line = [(-1, 1.5), (3.5, 1.5)]
    shapely_line = LineString(line)
    check = shapely_line.within(shapely_poly)
    test_inter = shapely_line.intersects(brep_test)
    test_inter_01 = shapely_line.intersection(brep_test)
    individual_points = [(pt.x, pt.y) for pt in test_inter_01]
    ...
    cross_vts, cross_edgs = trs_.create_cross_section_dtm(edges, source, receiver, source_tr, receiver_tr)
    trs_.create_cross_section_dsm(cross_vts, cross_edgs, None)
