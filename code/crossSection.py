...
class Tin:
    ...
    def side_test(self, pa, pb, pc):        #pa, pb, pc = index numbor of P
        ...
        """
        return ((self.pts[pa][0] - self.pts[pc][0]) * 
               (self.pts[pb][1] - self.pts[pc][1]) - 
               (self.pts[pb][0] - self.pts[pc][0]) * 
               (self.pts[pa][1] - self.pts[pc][1]))
    ...
    def point_in_triangle(self, p_source, tr):
        ...
        Output:
        """
        d1 = self.side_test(self.trs[tr][0], self.trs[tr][1], p_source)
        d2 = self.side_test(self.trs[tr][1], self.trs[tr][2], p_source)
        d3 = self.side_test(self.trs[tr][2], self.trs[tr][0], p_source)
    ...
    def find_source_triangle(self, tr_init, p_source):
        ...
        while(True):
            ...
            if(d1 >= 0 and d2 >= 0 and d3 >= 0): 
                return tr
            # when one the the tests returns negative, we have to go to the enxt triangle, first find the direction to go to:
...
def main(sys_args):
    ...
    trs = Tin(trs, vertices)
    # find source triangle
    Tin.find_source_triangle(4, source):
